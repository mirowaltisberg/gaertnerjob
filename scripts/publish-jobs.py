"""Validate, merge and publish one complete horticulture job snapshot.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_gaertner_title, validate_raw_job_identity
...
def is_recent_real_job(job: Any, cutoff: date, today: date) -> bool:
    ...
    if validate_raw_job_identity(job) is not None:
        return False
    if classify_gaertner_title(title)["disposition"] != "ACCEPT":
        return False
...
def calculate_snapshot_delta(
    existing: dict[str, date | None],
    candidate_ids: set[str],
) -> tuple[set[str], set[str]]:
    """Return retained and stale IDs for the exhaustive trade-scoped row set."""
    retained_ids = set(existing) & candidate_ids
    return retained_ids, set(existing) - retained_ids


def delete_stale_jobs(client: Client, stale_ids: set[str]) -> None:
    if any(not isinstance(job_id, str) for job_id in stale_ids):
        raise PipelineError("stale job ID set contains a non-string value")
    exact_ids = sorted(stale_ids)
    for offset in range(0, len(exact_ids), DELETE_BATCH_SIZE):
        batch = exact_ids[offset : offset + DELETE_BATCH_SIZE]
        (
            client.table("jobs")
...
def publish(
    ...
    _, stale_ids = calculate_snapshot_delta(existing, fresh_ids)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish one validated horticulture job snapshot")
...
def main() -> int:
    ...
    try:
        ...
        if args.plan:
            ...
            retained_ids, stale_ids = calculate_snapshot_delta(existing, fresh_ids)
            legacy_ids = {
                job_id for job_id in existing if not job_id.startswith(ID_PREFIX)
            }
            print(
                "Plan only: "
                f"{len(existing)} existing {TRADE} rows, "
                f"{len(existing) - len(legacy_ids)} current-prefix rows, "
                f"{len(legacy_ids)} legacy/invalid-ID rows, "
                f"{len(jobs)} validated candidates, "
                f"{len(retained_ids)} retained IDs, "
                f"{len(stale_ids)} exhaustive trade-scoped rows eligible for prune."
            )
        ...
        print(f"Published {published} jobs and pruned {pruned} stale Gärtner rows.")
        return 0
