"""Scrape a fresh chunk of genuine Swiss horticulture jobs into JSON.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_gaertner_title
...
def is_relevant_job(job: dict) -> bool:
    """Return True only for clearly horticulture-related Swiss roles."""
    ...
    return classify_gaertner_title(title)["disposition"] == "ACCEPT"
...
DEFAULT_SEARCH_TERMS = [
    ...
    "Mitarbeiter Grünpflege Gartenbau",
]
...
def _scrape_jobs_worker(result_queue, scrape_kwargs: dict) -> None:
    ...
    # JobSpy can emit complete upstream URLs at ERROR level even with verbose=0.
    # Keep query and location details inside the artifact, never in CI logs.
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Swiss horticulture jobs")
    ...
    # Distribute search terms evenly so every configured CI worker does real work.
    if args.total_chunks > 1 and not args.query:
        selected = select_strided_chunk(queries, args.chunk, args.total_chunks)
        queries = [query for _, query in selected]
        indices = ", ".join(str(index) for index, _ in selected) or "none"
        print(
            f"Chunk {args.chunk + 1}/{args.total_chunks}: processing search "
            f"terms {indices} ({len(queries)} terms)\n"
        )
