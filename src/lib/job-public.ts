...
import { classifyGaertnerTitle, getGaertnerProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifyGaertnerTitle(input.title);
  if (
  ...
  const profile = getGaertnerProfile(classification.profile);
