...
export function classifyGaertnerTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-gaertner-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-gaertner-title" : "broad-role-with-gaertner-context",
...
export function isValidGaertnerJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getGaertnerProfile(profile: JobSafetyProfile) {
