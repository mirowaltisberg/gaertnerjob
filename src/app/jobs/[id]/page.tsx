...
function getDisplayJobId(job: JobListing): string {
  ...
  if (job.source === "scraped") {
    ...
    return `GAR-${hash.slice(0, 8).toUpperCase()}`;
  }
  return `GAR-${job.id.padStart(4, "0")}`;
}
...
function buildJobPostingSchema(job: JobListing): Record<string, any> {
  ...
  const schema: Record<string, any> = {
    ...
    hiringOrganization: {
      "@type": "Organization",
      name: "Arbeitgeber via gaertnerjob.ch",
...
export async function generateMetadata(props: JobDetailsPageProps): Promise<Metadata> {
  ...
  if (!job) {
    return {
      title: "Stelle nicht gefunden | gärtnerjob.ch",
  ...
  return {
    title: `${job.title} | gaertnerjob.ch`,
    description,
    alternates: {
